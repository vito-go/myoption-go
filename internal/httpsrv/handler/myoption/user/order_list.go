...
func (h *OrderList) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	handler.ServeHTTP(w, r, h)

}

func (h *OrderList) GetParam(r *http.Request) (*handler.ReqParam, error) {
	return handler.GetParam1(r, "offset", "limit")
}

type orderListData struct {
	Items      []*fd.OrdersBinaryOptionString `json:"items"`
	FieldNames []FieldName                    `json:"fieldNames"`
}

func (h *OrderList) GetRespBody(ctx context.Context, p *handler.ReqParam) *resp.HTTPBody {
	...
	result, err := h.RepoClient.AllDao().OrdersBinaryOption.ItemsByUserIdOffsetLimit(ctx, userId, int(offset), int(limit))
	if err != nil {
		mylog.Ctx(ctx).Error(err.Error())
		return resp.Err(ctx, httperr.ErrData.Error())
	}
	items := make([]*fd.OrdersBinaryOptionString, 0, len(result))
	for _, option := range result {
		items = append(items, option.ToFd())
	}
	return resp.DataOK(ctx, orderListData{Items: items,
		FieldNames: fieldNamesOrderList})
}
